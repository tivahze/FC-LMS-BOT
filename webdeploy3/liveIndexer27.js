...
Store.prototype.init=async function(){
  ...
  await this.pool.query(`INSERT INTO live_index27(platform,club_id,name,last_poll,latest_match,last_new_match,next_type,last_roster,state,errors,last_error)
    ...
    ON CONFLICT(platform,club_id) DO UPDATE SET name=EXCLUDED.name`,[PLATFORM]);
  ...
  console.log(`[LIVE27] adaptive live indexer ready clubs=${num(q?.total)} polled=${num(q?.polled)} hot=${num(q?.hot)} loop=${LOOP_SECONDS}s batch=${BATCH}`);
