...
async function pollWatched(store){if(store.mode!=='postgres')return;const now=Math.floor(Date.now()/1000),staleBefore=now-POLL_MINUTES*60,activeAfter=now-ACTIVE_DAYS*86400,coldBefore=now-COLD_HOURS*3600;const rows=await store.q(`SELECT w.platform,w.club_id,w.name,w.last_viewed,w.last_polled,COALESCE(MAX(m.ts),0) latest_match FROM watched_clubs w LEFT JOIN matches m ON m.platform=w.platform AND (m.home_club_id=w.club_id OR m.away_club_id=w.club_id) WHERE w.club_id ~ '^[0-9]+$' GROUP BY w.platform,w.club_id,w.name,w.last_viewed,w.last_polled HAVING COALESCE(w.last_polled,0)<$1 AND (COALESCE(MAX(m.ts),0)>$2 OR w.last_viewed>$2 OR (COALESCE(w.last_polled,0)=0 AND COALESCE(MAX(m.ts),0)=0) OR (COALESCE(MAX(m.ts),0)>0 AND COALESCE(w.last_polled,0)<$4)) ORDER BY CASE WHEN w.last_viewed>$2 THEN 0 ELSE 1 END,CASE WHEN COALESCE(MAX(m.ts),0)>$2 THEN 0 ELSE 1 END,CASE WHEN COALESCE(w.last_polled,0)=0 THEN 0 ELSE 1 END,COALESCE(w.last_polled,0) ASC,COALESCE(MAX(m.ts),0) DESC LIMIT $3`,[staleBefore,activeAfter,POLL_BATCH,coldBefore]);for(const row of rows){await pollOne(store,row);await sleep(gap())}}
async function seedWatched(store){
  if(store.mode!=='postgres')return;
  let cursor='',done=0;
  try{
    for(;;){
      const rows=await store.q('SELECT uid,platform,club_id,name FROM clubs WHERE uid>$1 ORDER BY uid ASC LIMIT 350',[cursor]);
      if(!rows.length)break;
      const last=rows[rows.length-1].uid;
      await store.pool.query(`INSERT INTO watched_clubs(platform,club_id,name,last_viewed,last_polled,first_seen)
        SELECT platform,club_id,name,0,0,EXTRACT(EPOCH FROM NOW())::BIGINT FROM clubs WHERE uid>$1 AND uid<=$2
        ON CONFLICT(platform,club_id) DO UPDATE SET name=CASE WHEN EXCLUDED.name<>'' THEN EXCLUDED.name ELSE watched_clubs.name END`,[cursor,last]);
      cursor=last;done+=rows.length;
      if(done%1400===0)console.log(`[HISTORY] background seed ${done} clubs`);
      await sleep(40);
    }
    const tracked=await store.one('SELECT COUNT(*) c FROM watched_clubs');
    console.log(`[HISTORY] background seed complete tracked=${num(tracked?.c)}`);
  }catch(e){console.warn('[HISTORY] background seed:',e.message)}
}
const previousInit=Store.prototype.init;Store.prototype.init=async function(){
  ...
  console.log(`[HISTORY] smart tracking ready immediately; tracked=${num(tracked?.c)} active=${POLL_MINUTES}m cold=${COLD_HOURS}h batch=${POLL_BATCH}; full seed background`);
  setTimeout(()=>seedWatched(this),800).unref();
