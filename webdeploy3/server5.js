...
async function serveMatch(platform,id,req,res){
  ...
  const canonical=`${baseUrl(req)}/fr/match/${encodeURIComponent(platform)}/${encodeURIComponent(id)}`,title=`${row.home_name} ${row.home_goals}-${row.away_goals} ${row.away_name} — FC Clubs Global`,desc=`Match FC 26 Clubs archivé : ${row.home_name} ${row.home_goals}-${row.away_goals} ${row.away_name}. Statistiques et performances joueurs.`;
...
if(pathname==='/api/crawl')return send(res,200,crawl);if(pathname==='/api/dashboard')return send(res,200,await store.dashboard());
  ...
  if(pathname==='/api/v9/live'){const limit=Number(u.searchParams.get('limit')||60),items=await store.v9Live(limit);return send(res,200,{items})}
