...
const files={
  ...
  '/fr/compare':['compare.html','text/html; charset=utf-8'],'/fr/compare/':['compare.html','text/html; charset=utf-8'],'/compare.js':['compare.js','application/javascript; charset=utf-8'],'/player-detail.js':['player-detail.js','application/javascript; charset=utf-8'],'/club-detail.js':['club-detail.js','application/javascript; charset=utf-8']
};
...
async function serveProfile(type,platform,id,req,res){
  ...
  const title=`${row.name} — Stats FC 26 Clubs | FC Clubs Global`,desc=isPlayer?`Statistiques FC 26 Clubs de ${row.name}${row.club_name?` avec ${row.club_name}`:''} : matchs, buts, passes, note, poste et performances récentes.`:`Statistiques FC 26 Clubs de ${row.name} : bilan, Skill, effectif, forme, derniers matchs et adversaires.`;
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(pathname==='/sitemap.xml'){
    ...
    const urls=[`${base}/`,`${base}/fr/player`,`${base}/fr/club`,`${base}/fr/rankings`,`${base}/fr/compare`,...players.map(x=>`${base}/fr/player/${encodeURIComponent(x.platform)}/${encodeURIComponent(x.player_id)}/${slug(x.name)}`),...clubs.map(x=>`${base}/fr/club/${encodeURIComponent(x.platform)}/${encodeURIComponent(x.club_id)}/${slug(x.name)}`)];res.writeHead(200,{'content-type':'application/xml; charset=utf-8','cache-control':'public, max-age=3600'});return res.end(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map(x=>`<url><loc>${xmlEsc(x)}</loc></url>`).join('')}</urlset>`)
  }
  if(pathname==='/api/health')return send(res,200,{ok:true,version:'v8',storage:store.mode});if(pathname==='/api/crawl')return send(res,200,crawl);if(pathname==='/api/dashboard')return send(res,200,await store.dashboard());
...
server.listen(PORT,HOST,()=>{console.log(`FC Clubs Global V8 on ${HOST}:${PORT} [${store.mode}]`);if(process.env.AUTO_MAX_CRAWL!=='0')setTimeout(()=>runCrawler(store),1200)});setInterval(()=>runCrawler(store),Math.max(1,Number(process.env.AUTO_CRAWL_LOOP_HOURS||2))*3600000).unref();
