...
async function searchPrefix(store,row){
  ...
  const shouldExpand=count>0&&(
    row.depth===1 ||
    (row.depth===2&&count>=3) ||
    (row.depth>=3&&(count>=EXPAND_MIN||maxReturned>=45))
  );
  let children=0;
  if(shouldExpand&&row.depth<MAX_DEPTH)children=await expand(store,row.platform,row.prefix,row.depth,count*10+maxReturned);
  const state=errors===2&&/EA 400/.test(active.lastError||'')?'skipped':'done';
  await store.pool.query(`UPDATE discovery_frontier SET state=$1,last_count=$2,last_run=EXTRACT(EPOCH FROM NOW())::BIGINT,priority=$3 WHERE platform=$4 AND prefix=$5`,[state,count,count*10+maxReturned,row.platform,row.prefix]);
  active.processed++;
  active.found+=count;
  ...
  try{
    ...
    const rows=await store.q(`SELECT platform,prefix,depth,priority FROM discovery_frontier
      ...
      ORDER BY priority DESC,depth ASC,
        CASE platform WHEN 'common-gen5' THEN 0 ELSE 1 END,
        last_run ASC,prefix ASC
      LIMIT $1`,[BATCH,SEARCH_PLATFORMS]);
    ...
    for(const row of rows){
      try{await searchPrefix(store,row)}catch(e){
        ...
        await store.pool.query(`UPDATE discovery_frontier SET state=CASE WHEN $3 LIKE '%EA 400%' THEN 'skipped' ELSE state END,last_run=EXTRACT(EPOCH FROM NOW())::BIGINT,priority=GREATEST(priority-5,0) WHERE platform=$1 AND prefix=$2`,[row.platform,row.prefix,e.message||'']);
      }
    ...
    const s=await store.one(`SELECT COUNT(*) total,
      ...
      COUNT(*) FILTER(WHERE state='skipped') skipped,
      COALESCE(SUM(last_count),0) hits FROM discovery_frontier`);
...
Store.prototype.init=async function(){
  ...
  UPDATE discovery_frontier SET state='skipped'
    WHERE state='pending' AND (NOT(platform=ANY($1::text[])) OR (depth>1 AND priority<=1));`,[SEARCH_PLATFORMS]);
  await seed(this);
  ...
  setTimeout(()=>cycle(this).catch(e=>console.warn('[DISCOVERY10]',e.message)),25000).unref();
...
Store.prototype.dashboard=async function(){
  ...
  let s={total:0,pending:0,done:0,skipped:0,hits:0};
